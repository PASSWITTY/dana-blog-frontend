...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.password) {
            ...
            newErrors.password = "Password should have at least 8 characters e.g ('@John123') ";
            valid = false;
        }
        else {
            newErrors.password = "";
        ...
        if (valid) {
            const response = await SpacesApi.auth.login(formData)
            // console.log(response)
            if (response.data.status == 'success') {
                localStorage.setItem("user", JSON.stringify(response.data.data));
                if (response.data.data.role == 'admin') {
                    navigate('/admin/dashboard');
                } if (response.data.data.role == 'author') {
                    navigate("/author/dashboard");
                }
            } else {
                alert("Wrong credentiials")
            }
        }
    }
    return (
